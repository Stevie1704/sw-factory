...
func runEvaluation(ctx context.Context, args []string, defaultConfigPath string, output, errorsOutput io.Writer) int {
	...
	for _, summary := range result.Summaries {
		if !writeOutput(output, errorsOutput, "run: %s outcome=%s started=%s completed=%s wall=%s stages=%v versions=%v invocations=%d gates=%d check_repairs=%d test_revisions=%d review_revisions=%d review_diff_bytes=%d review_changed_lines=%d review_units=%d review_largest_unit_bytes=%d review_invocations=%d review_duration=%s review_findings=%v review_incomplete_units=%d review_cannot_proceed_units=%d review_over_budget=%d budget_exhausted=%t exemptions=%v escalations=%v blockers=%v usage_available=%t\n", summary.RunID, summary.Outcome, summary.StartedAt.Format(time.RFC3339Nano), summary.CompletedAt.Format(time.RFC3339Nano), summary.TotalWallTime, summary.StageDurations, summary.InvocationVersions, summary.InvocationCount, summary.GateCount, summary.CheckRepairCount, summary.TestRevisionCount, summary.ReviewRevisionCount, summary.ReviewDiffBytes, summary.ReviewChangedLines, summary.ReviewUnitCount, summary.ReviewLargestUnitBytes, summary.ReviewInvocationCount, summary.ReviewDuration, summary.ReviewFindingCounts, summary.ReviewIncompleteUnitCount, summary.ReviewCannotProceedUnitCount, summary.ReviewOverBudgetCount, summary.BudgetExhausted, summary.Exemptions, summary.EscalationCategories, summary.RepeatedBlockers, summary.Usage.Available) {
			return 1
		}
	...
	if !writeOutput(output, errorsOutput, "aggregate: runs=%d outcomes=%v outcome_rates=%v success_rate=%.4f budget_exhaustion_rate=%.4f total_wall=%s average_wall=%s stages=%v invocations=%d gates=%d check_repairs=%d test_revisions=%d review_revisions=%d review_diff_bytes=%d review_changed_lines=%d review_units=%d review_largest_unit_bytes=%d review_invocations=%d review_duration=%s review_findings=%v review_incomplete_units=%d review_cannot_proceed_units=%d review_over_budget=%d escalations=%v escalation_rates=%v dispositions=%v usage_available_runs=%d usage_cost_reported_runs=%d\n", aggregate.RunCount, aggregate.OutcomeCounts, aggregate.OutcomeRates, aggregate.SuccessRate, aggregate.BudgetExhaustionRate, aggregate.TotalWallTime, aggregate.AverageTotalWallTime, aggregate.StageDurations, aggregate.InvocationCount, aggregate.GateCount, aggregate.CheckRepairCount, aggregate.TestRevisionCount, aggregate.ReviewRevisionCount, aggregate.ReviewDiffBytes, aggregate.ReviewChangedLines, aggregate.ReviewUnitCount, aggregate.ReviewLargestUnitBytes, aggregate.ReviewInvocationCount, aggregate.ReviewDuration, aggregate.ReviewFindingCounts, aggregate.ReviewIncompleteUnitCount, aggregate.ReviewCannotProceedUnitCount, aggregate.ReviewOverBudgetCount, aggregate.EscalationCounts, aggregate.EscalationRates, aggregate.DispositionCounts, aggregate.UsageAvailableRuns, aggregate.UsageCostReportedRuns) {
		return 1
	}
