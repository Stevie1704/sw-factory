...
Scheduling a repeat was rejected because the original protocol targeted the pre-M3 minimum path and pinned historical packets, worker revisions, and harness versions. Recreating its disposable matched pairs would require substantial manual operator effort to measure a system the repository no longer ships; any future direct-harness comparison should use a new protocol against the current product rather than control runtime authorization through issue #26.
