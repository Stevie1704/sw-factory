...
Native process state is durable in the role volume. Fresh launch is idempotent
after response loss, exact-session resume may replace only a lost or exited
process, and cancellation/finalization are idempotent. Recovery therefore
distinguishes an active process, an exited process with a report, and a lost
process requiring the existing bounded resume policy without terminal
identities.
...
contract checks become part of Docker-backed diagnosis. A mixed repository
still needs the interactive terminal until issue #164 and issue #165 complete;
the coexistence is deliberate and bounded by those follow-up issues.
