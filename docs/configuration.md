...
`factory doctor` reports the `caches` check as a blocking finding when a
declared cache has no host mapping, when host configuration maps a cache the
repository does not declare, or when a mapped directory resolves outside
`cache_root`. A worker launch, gate run, or check repair fails closed for the
same reason, so a repository commit that adds a cache name after registration
cannot receive a defaulted host path.
