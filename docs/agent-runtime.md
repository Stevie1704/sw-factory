...
disabled in the frozen repository policy, the coordinator records the objection
and pauses for a human. When enabled, the coordinator resumes the original test
session with the current implementation context. The test role returns an
...
failure, or an objection after the configured revision budget pauses for human
disposition.
